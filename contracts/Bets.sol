...
contract Bets is ReentrancyGuard, GameOracle {
    ...
    function takeBet(uint256 _betIndex) external nonReentrant {
        ...
        if(bet.accountB == address(0)) {
            ...
            uint256 betPriceB = 10 - bet.betPrice;
    ...
    function _createBetA(
        ...
        uint256 _betPriceB = 10 - bet.betPrice;
    ...
    function cancelBetA(
        ...
        if (beforeGame) {
            if (betMatch) {
                ...
                _createBetB((10-bet.betPrice), bet.contractAmount);
    ...
    function cancelBetB(
        ...
        if (beforeGame) {
            if (betMatch) {
                delete bets[_betIndex];
                _createBetA((10-bet.betPrice), bet.contractAmount);
                transferStake(msg.sender, _stakedAmount);
            } else if (noMatch) {
                delete bets[_betIndex];
                transferStake(msg.sender, _stakedAmount);
